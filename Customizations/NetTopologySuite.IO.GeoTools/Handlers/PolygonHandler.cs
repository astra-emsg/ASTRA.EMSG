    ...
    public class PolygonHandler : ShapeHandler
    {
        ...
        public override IGeometry Read(BigEndianBinaryReader file, IGeometryFactory geometryFactory)
        {
            ...
            for (int i = 0; i < holes.Count; i++)
            {
                ...
                Envelope testEnv = testRing.EnvelopeInternal as Envelope;
                Coordinate testPt = testRing.GetCoordinateN(0) as Coordinate;
                ILinearRing tryRing;
                for (int j = 0; j < shells.Count; j++)
                {
                    tryRing = shells[j];
                    Envelope tryEnv = tryRing.EnvelopeInternal as Envelope;
