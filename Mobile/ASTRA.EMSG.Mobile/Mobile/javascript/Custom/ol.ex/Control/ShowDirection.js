...
OpenLayers.Control.ShowDirection = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function (lineLayer, options) {
        ...
        OpenLayers.Renderer.symbol.fillArrow = [0, 3, 1, 0, 2, 3, 0, 3];
        OpenLayers.Renderer.symbol.rect = [0, 0, 10, 1,10,1,10,0, 0, 0];
        //ir red=#7D1919
        if (!this.styleMap) {
            this.styleMap = new OpenLayers.StyleMap({ 'strokeWidth': 5, 'strokeColor': '#00ccff', graphicName: "arrow", rotation: "${angle}", pointRadius: 10 });
            //this.styleMap = new OpenLayers.StyleMap({ 'fillColor': '#000000', 'strokeWidth': 3, 'strokeColor': '#000000', graphicName: "rect", rotation: "${angle}", pointRadius: 5 });
        }
    },
    ...
    update: function () {
        if (this.map == null || this.events == null) return;
        this.layer.removeAllFeatures();        
        var index = this.map.getLayerIndex(this.lineLayer);
        this.map.setLayerIndex(this.layer, index);
