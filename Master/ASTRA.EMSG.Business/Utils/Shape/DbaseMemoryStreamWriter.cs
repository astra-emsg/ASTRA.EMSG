...
namespace ASTRA.EMSG.Business.Utils.Shape
{
    ...
    /// </remarks>
    public class DbaseMemoryStreamWriter
    {
        readonly BinaryWriter _writer;
        private MemoryStream dbfStream;
        ...
        public DbaseMemoryStreamWriter()
        {
            dbfStream = new MemoryStream();
            this.encoding = Encoding.UTF8;
            _writer = new BinaryWriter(dbfStream, this.encoding);
        }
        ...
        /// </summary>
        /// <param name="filename"></param>
        public DbaseMemoryStreamWriter(Encoding encoding)
        {
            dbfStream = new MemoryStream();
        ...
        public MemoryStream GetDbfStream()
        {            
        ...
        public void Write(string text, int length)
        {
            ...
            string dbaseString = this.BinarySubstring(text, length);

            // will extra chars get written??
            foreach (char c in dbaseString)
                _writer.Write(c);

            int extraPadding = length - encoding.GetByteCount(dbaseString);
            for (int i = 0; i < extraPadding; i++)
                _writer.Write((byte)0x20);
        }

        public string BinarySubstring(string text, int byteLength)
        {
            string ret = text;
            ...
            {
                int cutoff = totalbytes - byteLength;
                int newlength = (ret.Length - cutoff)>0 ?(ret.Length - cutoff):0;
                string left = ret.Substring(0, newlength);
                if (encoding.GetByteCount(left) == byteLength)
                {
                    //all characters which are represeneted by more than one byte are on the left side of the string or there are no special characters 
                    return left;
                }
                else
                {
                    //one ore more character with special encoding are on the right side of the string making the left side smaller than it needs to be
                    int remainingcharactersCount = ret.Length - newlength;
                    for (int i = 0; i < remainingcharactersCount; i++)
                    {
                        char character = ret[newlength + i];
                        int charbytes = encoding.GetByteCount(new char[]{character});
                        int stringbytes = encoding.GetByteCount(left);

                        if (charbytes + stringbytes > byteLength)
                        {
                            return left;
                        }
                        else
