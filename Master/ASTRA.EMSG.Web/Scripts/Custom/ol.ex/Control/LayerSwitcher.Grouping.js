(function () {
   ...
   , redraw: function () {
       ...
       for (var i = 0, len = layers.length; i < len; i++) {
           ...
           if (layer.displayInLayerSwitcher) {
               ...
               if (legend && legend.hasName(layer.params["LAYERS"]) && legend.showIcon) {
                   var name = layer.params["LAYERS"];
                   legendIcon = document.createElement("span");
                   addClass(legendIcon, "olButton legendIconSpan");
                   var cb = function (e) {
                       this.legend.onLegendIcon(this.name);
                       //OpenLayers.Event.stop(e);
                   };
                   observe(legendIcon, "click", bindAsEventListener(cb, { legend: legend, name: name }));
               };
       ...
       if (!($('#sliderTransparency').length)) {
           var transparency = 0;
           if (mapTransparency) {
               transparency = mapTransparency;
               var length = emsg.map.map.layers.length;
               for (var i = 0; i < length; i++) {
                   if (emsg.map.map.layers[i].isBaseLayer) {
                       emsg.map.map.layers[i].setOpacity((100 - transparency) / 100);
                   }
               }
           }
           var labelwidth = $(this.baseLbl).width();
           this.slider = $("<div id='sliderTransparency'></div>").insertAfter(this.baseLbl).slider({
               min: 0,
               max: 100,
               value: transparency
           }).width(this.getSliderWidth());
           var slideTooltip = $("<div id='slideTooltip' />")
                    .css({ position: 'absolute', top: 15, left: 0 })
                    .text(emsg.config.get("TEXT.LABEL.TRANSPARENCY") + ": " + transparency + "%")
                    .hide();

           this.slider.append(slideTooltip);

           this.slider.on("slidestart", function (event, ui) {
               slideTooltip.show();
           });
           ...
           this.slider.on("slide", function (event, ui) {
               slideTooltip.text(emsg.config.get("TEXT.LABEL.TRANSPARENCY") + ": " + ui.value + "%");
               var length = emsg.map.map.layers.length;
               for (var i = 0; i < length; i++) {
                   if (emsg.map.map.layers[i].isBaseLayer) {
                       emsg.map.map.layers[i].setOpacity((100 - ui.value) / 100);
                       emsg.map.map.events.triggerEvent("changetransparency", ui);
                   }
               }
           });
           //prevent events from bleeding to the map
           this.slider.bind("mousedown", function () {
               return false;
           });
           this.slider.bind("click", function () {
               return false;
           });
       }
       return this.div;
 ...
 , getSliderWidth: function () {
     ...
     var radioDivs = basediv.find('input.olButton');
     var labelDivs = basediv.find('label.labelSpan');
     var radiowidth = 0;
     var labelwidth = 0;
     radioDivs.each(function (index) {
         var elementwidth = $(this).width();
         if (elementwidth > radiowidth) {
             radiowidth = elementwidth;
         }
     });
     labelDivs.each(function (index) {
         var elementwidth = $(this).width();
         if (elementwidth > labelwidth) {
             labelwidth = elementwidth;
         }
     });
     return radiowidth + labelwidth;
 }
